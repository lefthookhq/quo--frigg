...
const BASE_URL = USE_DEV
    ? 'https://dev-public-api.openphone.dev/v1'
...
function makeRequest(method, path, body = null) {
    return new Promise((resolve, reject) => {
        ...
        const baseUrlWithoutVersion = USE_DEV
    ...
    const contacts = [
        {
            ...
            defaultFields: {
                ...
                emails: [
                    { name: 'email', value: `test1-${timestamp}@example.com` },
                ],
            },
            customFields: [],
        },
        {
            ...
            defaultFields: {
                ...
                emails: [
                    { name: 'email', value: `test2-${timestamp}@example.com` },
                ],
            },
            customFields: [],
        },
...
async function testSingleCreate() {
    ...
    const contact = {
        ...
        defaultFields: {
            ...
            emails: [
                { name: 'email', value: `single-${timestamp}@example.com` },
            ],
        },
        customFields: [],
    };
...
async function testDuplicatePhone() {
    ...
    console.log(
        'Result 1:',
        result1.status === 201 ? '✅ Created' : `❌ ${result1.status}`,
    );
    console.log(
        'Result 2:',
        result2.status === 201 ? '✅ Created' : `❌ ${result2.status}`,
    );
...
async function testDuplicateExternalId() {
    ...
    console.log(
        '\n--- Creating Contact 2 with SAME externalId',
        sharedExternalId,
    );
    const contact2 = {
...
async function testWebhookEndpoints() {
    ...
    console.log(
        'v1/webhooks:',
        results.v1.webhooks.status === 200
            ? '✅ Works'
            : `❌ ${results.v1.webhooks.status || 'Error'}`,
    );
    console.log(
        'v1/webhooks/messages:',
        results.v1.messages.status === 200
            ? '✅ Works'
            : `❌ ${results.v1.messages.status || 'Error'}`,
    );
    console.log(
        'v1/webhooks/calls:',
        results.v1.calls.status === 200
            ? '✅ Works'
            : `❌ ${results.v1.calls.status || 'Error'}`,
    );
    console.log('');
    console.log(
        'v2/webhooks:',
...
async function testWebhookV2CreateDelete() {
    ...
    try {
        ...
        if (
            createMessageResult.status === 201 &&
            createMessageResult.data?.data?.id
        ) {
            ...
            console.log(
                `   Events: ${createMessageResult.data.data.events?.join(', ')}`,
            );
        } else {
        ...
        if (createCallResult.status === 201 && createCallResult.data?.id) {
            ...
            console.log(
                `   Events: ${createCallResult.data.events?.join(', ')}`,
            );
        } else {
        ...
        if (listResult.status === 200) {
            const messageWebhook = listResult.data?.data?.find(
                (w) => w.id === createdMessageWebhookId,
            );
            const callWebhook = listResult.data?.data?.find(
                (w) => w.id === createdCallWebhookId,
            );
        ...
        if (createdMessageWebhookId) {
            ...
            if (
                deleteMessageResult.status === 204 ||
                deleteMessageResult.status === 200
            ) {
                console.log(
                    `✅ Message webhook deleted successfully! (Status: ${deleteMessageResult.status})`,
                );
            } else {
                console.log(
                    `⚠️  Message webhook deletion status: ${deleteMessageResult.status}`,
                );
            }
        }
        ...
        if (createdCallWebhookId) {
            ...
            if (
                deleteCallResult.status === 204 ||
                deleteCallResult.status === 200
            ) {
                console.log(
                    `✅ Call webhook deleted successfully! (Status: ${deleteCallResult.status})`,
                );
            } else {
                console.log(
                    `⚠️  Call webhook deletion status: ${deleteCallResult.status}`,
                );
            }
        }
        ...
        if (verifyResult.status === 200) {
            ...
            const callWebhook = verifyResult.data?.data?.find(
                (w) => w.id === createdCallWebhookId,
            );

            if (!messageWebhook && createdMessageWebhookId)
                console.log(`✅ Message webhook removed from list`);
            if (!callWebhook && createdCallWebhookId)
                console.log(`✅ Call webhook removed from list`);
        }
...
(async () => {
    try {
        ...
        console.log(
            'Environment:',
            USE_DEV
                ? 'DEV (dev-public-api.openphone.dev)'
                : 'PRODUCTION (api.openphone.com)',
        );
