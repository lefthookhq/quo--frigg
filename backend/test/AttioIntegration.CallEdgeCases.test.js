...
describe('AttioIntegration - Call Edge Cases (TDD)', () => {
    ...
    describe('Domain Rule: Sona-handled Calls (AI Assistant)', () => {
        it('should log Sona-answered call with AI assistant indicator', async () => {
            // Arrange - Domain Event: Call answered by Sona AI assistant
            // Note: Assuming Sona calls have answeredBy: 'sona' or similar identifier
            const webhookData = {
                type: 'call.completed',
                data: {
                    object: {
                        ...
                        phoneNumberId: 'pn-456',
                        userId: 'user-789',
                        answeredAt: '2025-01-15T15:00:00Z', // Sona calls ARE answered
                        answeredBy: 'sona', // AI assistant identifier
                        createdAt: '2025-01-15T15:00:00Z',
                    },
                    deepLink: 'https://app.openphone.com/calls/call-sona-001',
                },
            };
            ...
            mockAttioApi.api.createNote.mockResolvedValue({
                data: { id: { note_id: 'note-sona-123' } },
            });

            // Act
            await integration._handleQuoCallEvent(webhookData);

            // Assert - Sona-handled call logged with AI indicator
            const noteContent = mockAttioApi.api.createNote.mock.calls[0][0].content;
            expect(noteContent).toContain('Incoming answered by Sona AI');
            expect(noteContent).toContain('Recording (2:00)'); // 120 seconds
            expect(noteContent).toContain('[View the call activity in Quo]');
        });

        it('should handle Sona call with auto-generated transcript in enrichment', async () => {
            // Arrange - Domain Event: Sona call with AI-generated summary (REAL structure)
            const webhookData = {
                type: 'call.summary.completed',
                data: {
                    object: {
                        callId: 'call-sona-002',
                        summary: [
                            'Customer inquired about pricing',
                            'AI assistant provided standard pricing info',
                            'Customer requested human callback',
                        ],
                        nextSteps: ['Sales team to follow up within 24 hours'],
                        status: 'completed',
                    },
                    deepLink: 'https://app.openphone.com/calls/call-sona-002',
                },
            };

            integration.getMapping.mockResolvedValue({
                noteId: 'note-sona-old',
                callId: 'call-sona-002',
                attioContactId: 'attio-contact-123',
            });
            ...
            mockQuoApi.api.getCall.mockResolvedValue({
                data: {
                    ...
                    phoneNumberId: 'pn-456',
                    userId: 'user-sona',
                    answeredAt: '2025-01-15T16:00:00Z',
                    answeredBy: 'sona',
                    aiHandled: 'ai-agent', // Real field from Quo API
            ...
            mockQuoApi.api.getCallRecordings.mockResolvedValue({
                data: [
                    {
                        url: 'https://storage.example.com/sona-call-002.mp3',
                        duration: 180,
                    },
                ],
            });
            ...
            mockAttioApi.api.createNote.mockResolvedValue({
                data: { id: { note_id: 'note-sona-enriched' } },
            });

            // Act
            await integration._handleQuoCallSummaryEvent(webhookData);

            // Assert - Enriched note shows "Handled by Sona" and includes summary
            const enrichedNote = mockAttioApi.api.createNote.mock.calls[0][0];
            expect(enrichedNote.title).toContain('☎️  Call '); // Not "Call Summary:" for Sona calls
            expect(enrichedNote.content).toContain('Handled by Sona');
            expect(enrichedNote.content).toContain('Summary:');
            expect(enrichedNote.content).toContain('Customer inquired about pricing');
            expect(enrichedNote.content).toContain('AI assistant provided standard pricing info');
            expect(enrichedNote.content).toContain('Next Steps:');
            expect(enrichedNote.content).toContain('Sales team to follow up within 24 hours');
            expect(enrichedNote.content).toContain('https://storage.example.com/sona-call-002.mp3');
        });
    });
