...
class ZohoCRMIntegration extends IntegrationBase {
    static Definition = {
        ...
        modules: {
            zoho: {
                definition:
                    require('/Users/danielklotz/projects/lefthook/api-module-library/packages/v1-ready/zoho-crm')
                        .Definition,
            },
            quo: {
    ...
    async syncContactsToQuo(args) {
        try {
            ...
            for (const contact of zohoContacts.data?.slice(
                0,
                args.maxContacts || 10,
            ) || []) {
                try {
                    ...
                    const quoContactData =
                        await this.transformContactForQuo(contact);
    ...
    async getModules(args) {
        try {
            ...
            for (const module of modules.data?.details?.slice(
                0,
                args.maxModules || 10,
            ) || []) {
                try {
                    const fields = await this.zoho.api.getFields(
                        module.api_name,
                    );
            ...
            return {
                ...
                data: {
                    summary: {
                        ...
                        modulesWithFields: moduleData.filter((m) => !m.error)
                            .length,
                        totalFields: moduleData.reduce(
                            (sum, m) => sum + (m.fieldCount || 0),
                            0,
                        ),
                    },
                    moduleData,
                    ...
                    jsonSchema: {
                        ...
                        properties: {
                            module: {
                                ...
                                title: 'Module',
                                description:
                                    'Zoho CRM module to create record in',
                                enum: [
                                    'leads',
                                    'contacts',
                                    'deals',
                    ...
                    jsonSchema: {
                        ...
                        properties: {
                            ...
                            module: {
                                ...
                                title: 'Module',
                                description:
                                    'Specific module to search (leave empty for all)',
                                enum: [
                                    '',
                                    'leads',
                                    'contacts',
                            ...
                            criteria: {
                                ...
                                description:
                                    'Additional search criteria in JSON format',
                            },
                        },
                        required: ['term'],
                    },
                    ...
                    jsonSchema: {
                        ...
                        properties: {
                            maxModules: {
                                ...
                                title: 'Max Modules',
                                description:
                                    'Maximum number of modules to process',
                                minimum: 1,
                                maximum: 50,
                                default: 10,
                            },
                            maxFields: {
                                ...
                                description:
                                    'Maximum fields to retrieve per module',
                                minimum: 5,
                                maximum: 100,
                                default: 20,
                            },
