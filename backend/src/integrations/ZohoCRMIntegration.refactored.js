...
class ZohoCRMIntegration extends BaseCRMIntegration {
    ...
    static CRMConfig = {
        ...
        syncConfig: {
            ...
            initialBatchSize: 50,
            ongoingBatchSize: 25,
            supportsWebhooks: false, // Zoho CRM doesn't support webhooks in this setup
    ...
    async fetchPersonPage({ objectType, page, limit, modifiedSince, sortDesc = true }) {
        try {
            // Convert 0-indexed page to 1-indexed for Zoho API
            const zohoPage = page + 1;

            // Map objectType to Zoho API endpoint
            ...
            const params = {
                ...
                sort_by: 'Modified_Time', // Use Modified_Time for better sync control
            };

            // Add modification filter if provided
            if (modifiedSince) {
                // Zoho CRM uses specific date format
                params.modified_since = modifiedSince.toISOString().split('T')[0]; // YYYY-MM-DD format
            }
    ...
    async transformPersonToQuo(person) {
        ...
        return {
            ...
            customFields: {
                ...
                lastModified: person.Modified_Time,
                createdTime: person.Created_Time,
    ...
    async logSMSToActivity(activity) {
        try {
            // Find the contact/lead by external ID
            const person = await this.findPersonByExternalId(activity.contactExternalId);
            if (!person) {
            ...
            const activityData = {
                ...
                Who_Id: person.id, // Contact or Lead ID
                Activity_Type: 'SMS',
                Status: 'Completed',
     ...
     */
    async setupWebhooks() {
        // Zoho CRM webhook setup would go here
        // For now, we'll use polling fallback
        console.log('Zoho CRM webhooks not configured - using polling fallback');
    }
    ...
    async createRecord({ req, res }) {
        try {
            ...
            const result = await this.zoho.api[module.toLowerCase()].create(data);
            res.json(result);
        } catch (error) {
    ...
    async searchData({ req, res }) {
        try {
            ...
            const result = await this.zoho.api[module.toLowerCase()].search(criteria);
            res.json(result);
        } catch (error) {
